...
import { IRS_TY_2026, rulesForYear } from './irs-rules';

const DRAFT_NOTICE =
  'DRAFT — NOT FOR FILING. For official filing, order scannable forms at ' +
...
function draftBanner(doc: PDFKit.PDFDocument) {
...
export function generateW2Pdf(w2: any, res: Response) {
  ...
  function value(x: number, yy: number, w: number, text: string, bold = false) {
    doc.fontSize(9).font(bold ? 'Helvetica-Bold' : 'Helvetica').fillColor('#000')
      .text(text ?? '', x + 3, yy + labelH, { width: w - 6 });
  }
  function cell(x: number, yy: number, w: number, h: number) {
    doc.rect(x, yy, w, h).stroke();
  }
  function box(x: number, yy: number, w: number, h: number, boxLabel: string, desc: string, val: string, money = false) {
    cell(x, yy, w, h);
  ...
  cell(col1 + 260, y, 280, rowH); label(col1 + 260, y, 'c — Employer name, address, ZIP');
  value(col1 + 260, y, 280, `${w2.companyName ?? ''}\n${w2.companyAddress ?? ''}`, true);
  y += rowH;
  box(col1 + 260, y, 130, rowH, 'd', 'Control number', w2.controlNumber ?? '');
  cell(col1 + 390, y, 150, rowH); label(col1 + 390, y, 'e — Employee first name, MI, last');
  value(col1 + 390, y, 150, `${w2.firstName ?? ''} ${w2.lastName ?? ''}`, true);
  y += rowH;
  cell(col1, y, 540, rowH); label(col1, y, 'f — Employee address and ZIP');
  value(col1, y, 540, w2.address ?? '');
  y += rowH + 4;

  // Numbered boxes (two-column layout)
  const half = (colW: number) => colW / 2;
  const left = col1, right = col1 + 280, boxW = 270, numH = 24;
  const numberedBoxes: Array<[string, string, string, boolean]> = [
  ...
  cell(left, y, 540, numH * 2); label(left, y, '12 — See instructions for Box 12 codes (up to 4)');
  ...
  // Box 13 checkboxes
  cell(left, y, 270, numH); label(left, y, '13 — Checkboxes');
  doc.fontSize(9).font('Helvetica').fillColor('#000')
    .text(`${w2.retirementPlan ? '☑' : '☐'} Retirement plan    ${w2.thirdPartySickPay ? '☑' : '☐'} Third-party sick pay`,
      left + 10, y + labelH + 2);
  // Box 14 — Other
  cell(right, y, 270, numH); label(right, y, '14 — Other (state-specific)');
  value(right, y, 270, w2.box14Other ?? '');
  y += numH;

  // State/local grid (15–20)
  const stateRow = (boxNo: string, lbl: string, lbl2: string) => {
    cell(left, y, 60, numH); label(left, y, `Box ${boxNo}`); value(left, y, 60, w2[`box${boxNo}`] ?? '');
    cell(left + 60, y, 480, numH); label(left + 60, y, lbl); value(left + 60, y, 480, w2[`box${boxNo}Val`] ?? '');
    y += numH;
  };
  ...
  cell(left + 260, y, 280, numH); label(left + 260, y, '18 — Local wages, tips, etc.');
  value(left + 260, y, 280, fmtMoney(w2.localWages));
  y += numH;
  box(left, y, 130, numH, '19', 'Local income tax', fmtMoney(w2.localTax), true);
  cell(left + 130, y, 410, numH); label(left + 130, y, '20 — Locality name');
...
export function generate1099Pdf(f: any, res: Response) {
  ...
  const halfW = 270;
  cell(col1, y, halfW, rowH * 2); label(col1, y, "PAYER'S name, address, ZIP, phone");
  value(col1, y, halfW, `${f.companyName ?? ''}\n${f.companyAddress ?? ''}`, true);
  box(col1, y + rowH, halfW, rowH, '', 'Payer TIN (EIN)', f.companyEin ?? '');
  cell(col1 + halfW, y, halfW, rowH * 2); label(col1 + halfW, y, "RECIPIENT'S name and address");
  const rname = f.businessName
    ? `${f.businessName} (${f.firstName ?? ''} ${f.lastName ?? ''})`
    : `${f.firstName ?? ''} ${f.lastName ?? ''}`;
  value(col1 + halfW, y, halfW, `${rname}\n${f.address ?? ''}`, true);
  box(col1 + halfW, y + rowH, halfW, rowH, '', `Recipient TIN (${(f.tinType ?? 'ssn').toUpperCase()})`,
    f.tinLast4 ? `XXX-XX-${f.tinLast4}` : '');
  y += rowH * 2;
...
export function generate1099MiscPdf(f: any, res: Response) {
  const doc = new PDFDocument({ size: 'LETTER', margin: 36 });
  doc.pipe(res);
  draftBanner(doc);
  ...
  function value(x: number, yy: number, w: number, text: string) {
    doc.fontSize(9).font('Helvetica').fillColor('#000').text(text ?? '', x + 3, yy + 11, { width: w - 6 });
  }
  function cell(x: number, yy: number, w: number, h: number) { doc.rect(x, yy, w, h).stroke(); }
  function box(x: number, yy: number, w: number, h: number, boxLabel: string, desc: string, val: string, money = false) {
    cell(x, yy, w, h); label(x, yy, `Box ${boxLabel} — ${desc}`);
    value(x, yy, w, money ? `$${fmtMoney(Number(val) || 0)}` : (val ?? ''));
  }

  cell(col1, y, w540, rowH * 2); label(col1, y, "PAYER & RECIPIENT (name, address, TIN)");
  ...
  y += rowH * 2;
  cell(col1, y, w540, rowH); label(col1, y, 'Account number'); value(col1, y, w540, f.accountNumber ?? '');
  y += rowH + 2;
...
export function generateW3Pdf(records: any[], company: any, taxYear: number, res: Response) {
  const doc = new PDFDocument({ size: 'LETTER', margin: 36 });
  doc.pipe(res);
  draftBanner(doc);
  ...
  const col1 = 36;
  let y = doc.y;
  function label(x: number, yy: number, t: string) {
    doc.fontSize(7).font('Helvetica').fillColor('#444').text(t, x + 3, yy + 2);
  }
  function value(x: number, yy: number, w: number, t: string, bold = false) {
    doc.fontSize(9).font(bold ? 'Helvetica-Bold' : 'Helvetica').fillColor('#000')
      .text(t, x + 3, yy + 11, { width: w - 6 });
  }
  function cell(x: number, yy: number, w: number, h: number) { doc.rect(x, yy, w, h).stroke(); }
  function box(x: number, yy: number, w: number, h: number, lbl: string, desc: string, val: string, money = false) {
    cell(x, yy, w, h); label(x, yy, `${lbl} — ${desc}`);
    value(x, yy, w, money ? `$${fmtMoney(Number(val) || 0)}` : val, true);
  }
  const rowH = 26;
  ...
  y += rowH;
  cell(col1, y, 540, rowH); label(col1, y, 'e — Employer address');
  ...
  doc.fillColor('#cc0000').text(
    'DRAFT — NOT FOR FILING. Required ONLY for paper-filed W-2s. ' +
    'If you e-file W-2s via SSA BSO (https://www.ssa.gov/bso), do NOT submit a W-3.',
    { align: 'center' }
  );
  doc.end();
}
...
export function generate1096Pdf(records: any[], company: any, taxYear: number, formType: string, res: Response) {
  const doc = new PDFDocument({ size: 'LETTER', margin: 36 });
  doc.pipe(res);
  draftBanner(doc);
  ...
  let totalAmount = 0;
  for (const r of records) totalAmount += r.box1NonemployeeComp || r.box1Rents || 0;
  ...
  y += rowH;
  cell(col1, y, 540, rowH); label(col1, y, 'Address'); value(col1, y, 540, company?.address ?? '');
  y += rowH;
  ...
  doc.fontSize(7).font('Helvetica').text(
    'If your principal business is in OR, WA, ID, MT, ND, SD, WY, CO, UT, NV, AZ, NM, ' +
    ...
    'With 1099-NEC: due Jan 31. With other 1099s: due Feb 28.',
    col1, y, { width: 540 }
  );
  y += 90;
  ...
  doc.fillColor('#cc0000').text(
    'DRAFT — NOT FOR FILING. Paper transmittal ONLY. Do not use for e-filed 1099s.',
    { align: 'center' }
  );
  doc.end();
}
...
export function generate941Worksheet(quarter: number, taxYear: number, totals: {
  wages: number;
  fedTax: number;
  ssWages: number;
  ssTax: number;
  ...
  function cell(x: number, yy: number, w: number, h: number) { doc.rect(x, yy, w, h).stroke(); }
  function box(x: number, yy: number, w: number, h: number, lbl: string, desc: string, val: string, money = true) {
    cell(x, yy, w, h); label(x, yy, `Line ${lbl} — ${desc}`);
    value(x, yy, w, money ? `$${fmtMoney(Number(val) || 0)}` : val, true);
  }
  ...
  y += 12;
  box(col1, y, 270, rowH, '4', 'SS taxable wages (cap $' + rules.SS_WAGE_BASE.toLocaleString() + ')',
    fmtMoney(Math.min(line3, rules.SS_WAGE_BASE)), true);
  ...
  y += rowH;
  box(col1, y, 270, rowH, '5d', 'Additional Medicare Tax (0.9% over $200k)',
    fmtMoney(line5d), true);
  ...
  y += 12;
  const monthlySchedule = line12 < IRS_TY_2026.DEPOSIT_THRESHOLD_QUARTERLY;
  doc.fontSize(8).font('Helvetica').text(
    monthlySchedule
      ? '• Monthly depositor: pay with return (line 12 < $2,500).'
      : '• Semiweekly or monthly depositor (line 12 ≥ $2,500). Complete Schedule B (Form 941).',
    col1, y, { width: 540 }
  );
  y += 14;
  doc.text(
    `• If line 12 reaches $${IRS_TY_2026.DEPOSIT_NEXT_DAY_TRIGGER.toLocaleString()}+ at any point, deposit by next business day.`,
    col1, y, { width: 540 }
  );
  y += 20;
  ...
  doc.fontSize(8).font('Helvetica-Bold').text(
    `Q${quarter} ${taxYear} filing deadline: ${deadlines[quarter]}`,
    col1, y
  );
  y += 16;
  ...
  doc.fillColor('#cc0000').text(
    'DRAFT — NOT FOR FILING. File official 941 via IRS e-file (https://www.irs.gov/efile) or mail to the IRS service center.',
    { align: 'center' }
  );
  doc.end();
}
...
export function generate940Worksheet(taxYear: number, totals: {
  ...
  y += rowH;
  box(col1, y, 540, rowH, '4', 'Gross FUTA wages (line 3 capped at $7,000/employee)',
    `$${fmtMoney(Math.min(line3, rules.FUTA_WAGE_BASE))}`, true);
  y += rowH;
  ...
  y += rowH;
  box(col1, y, 540, rowH, '7', 'FUTA tax before adjustments (line 4 − line 6)',
    fmtMoney(line7), true);
  y += rowH;
  ...
  y += 14;
  doc.fontSize(8).font('Helvetica').text(
    'File via IRS e-file (https://www.irs.gov/efile) or mail to the Department of the Treasury, ' +
    'Internal Revenue Service, Kansas City, MO 64999-0044.',
    col1, y, { width: 540 }
  );
  y += 30;
  ...
  doc.fontSize(7).font('Helvetica-Oblique').fillColor('#666')
    .text(`OMB No. 1545-0029  •  ${SOURCE_LINE}`, { align: 'center' });
  doc.fillColor('#cc0000').text(
    'DRAFT — NOT FOR FILING. This worksheet is for calculation reference only. ' +
    'Use IRS e-file or mail the official Form 940 to file.',
    { align: 'center' }
  );
  doc.end();
}
