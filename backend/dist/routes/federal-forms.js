...
router.get('/w3/:year', (req, res) => {
    ...
    const records = db_1.default
        .prepare(`SELECT w.* FROM w2_records w
    ...
    res.setHeader('Content-Disposition', `inline; filename="W3-DRAFT-${year}-${company?.name?.replace(/\s+/g, '-')}.pdf"`);
    (0, pdf_1.generateW3Pdf)(records, company, year, res);
    (0, db_2.logAudit)({
...
router.get('/1096/:year', (req, res) => {
    ...
    const company = db_1.default
        ...
        .get(req.user.companyId);
    const records = db_1.default
        .prepare(`SELECT f.* FROM form1099_records f
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="1096-DRAFT-${year}-${formType}-${company?.name?.replace(/\s+/g, '-')}.pdf"`);
    (0, pdf_1.generate1096Pdf)(records, company, year, formType, res);
    (0, db_2.logAudit)({
        companyId: req.user.companyId,
        userId: req.user.userId,
...
router.get('/941/:year/:quarter', (req, res) => {
    ...
    const totals = db_1.default
       ...
       WHERE pr.companyId = ?
        ...
        .get(req.user.companyId, quarterStart.toISOString().slice(0, 10), quarterEnd.toISOString().slice(0, 10));
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="941-WORKSHEET-Q${quarter}-${year}.pdf"`);
    (0, pdf_1.generate941Worksheet)(quarter, year, totals941, res);
    (0, db_2.logAudit)({
        companyId: req.user.companyId,
        userId: req.user.userId,
...
router.get('/940/:year', (req, res) => {
    ...
    const totals = db_1.default
        .prepare(`SELECT COALESCE(SUM(gross), 0) as wages
       FROM payroll_lines pl
       JOIN payroll_runs pr ON pr.id = pl.payrollRunId
       WHERE pr.companyId = ? AND strftime('%Y', pr.payDate) = ?`)
        .get(req.user.companyId, String(year));
    ...
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `inline; filename="940-WORKSHEET-${year}.pdf"`);
    (0, pdf_1.generate940Worksheet)(year, totals940, res);
    (0, db_2.logAudit)({
        companyId: req.user.companyId,
        userId: req.user.userId,
...
//# sourceMappingURL=federal-forms.js.map