...
router.post('/', (req, res) => {
    ...
    const r = db_1.default
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(req.user.companyId, i.customerName, i.customerEmail ?? null, i.amount, i.status, i.issueDate, i.dueDate, i.memo ?? null);
    (0, db_2.logAudit)({
...
router.patch('/:id', (req, res) => {
    ...
    db_1.default.prepare(`UPDATE invoices SET ${sets.join(', ')} WHERE id = ? AND companyId = ?`).run(...vals);
    (0, db_2.logAudit)({
        companyId: req.user.companyId,
        userId: req.user.userId,
